...
build_all()
{
    ...
    create_from_github libbitcoin libbitcoin-protocol v3.8.0 "yes"
    build_from_github libbitcoin-protocol "$PARALLEL" false "yes" "${BITCOIN_PROTOCOL_OPTIONS[@]}" "$@"
    if [[ ! ($CI == true) ]]; then
        create_from_github libbitcoin libbitcoin-server v3.8.0 "yes"
